<template>
  <div class="space-y-6" data-testid="programs">
    <div v-if="programs.length === 0" class="text-center py-8" data-testid="programs-empty">
      <p class="text-gray-500">No race programs available. Generate horses and programs first.</p>
    </div>

    <div v-for="program in programs" :key="program.id" :id="`program-${program.id}`" class="pb-6 border-b-1 last:border-b-0" :data-testid="`program-${program.id}`">
      <h4 class="text-lg font-semibold mb-4 text-center" data-testid="program-name">{{ program.name }}</h4>
